...
* O `enum` é apensar para padronizar nossas informações;
...
O modelo do app da Loja XPTO é o mesmo do anterior, então mãos à obra! O projeto será um Single View Application, em Objective-c, para iPhone. Não utilizaremos nenhuma interface nesse projeto pois iremos utilizar `UIAlertView` para exemplificar.
...
Primeiro o `locationManager` é instanciado e dizemos à ele que o delegate é a classe.

Criamos o `uuid` com **o mesmo** id que utilizamos no nosso "beacon", pois é ele que iremos monitorar. O mesmo acontece com o `identifier`.

o `beaconRegion` é inicializado com o uuid, major e identificador apenas, porque não podemos restringir a busca pelo minor, que são os diferentes tipos de departamento.
